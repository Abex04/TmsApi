...
// In-memory for the lab. Same interface would point at Redis/SQL in production -
// the controller and worker never know which implementation is behind it.
public class InMemoryTranscriptStatusStore : ITranscriptStatusStore
{
    ...
    // Enforces the state machine: a transition is only legal from the
    // exact state the caller expects (allowedFrom). Anything else throws,
    // which is what catches a bad "Ready back to Queued" attempt.
